...
public class JobService {
    ...
    public ResponseObject delete(Long userId, Long jobId) {
        ...
        if (organization == null) {
            return ResponseObject.message("not access");
        }
        Job job = jobRepository.findById(jobId).orElse(null);
        if (job == null) {
            return ResponseObject.message("Job not found");
        }
        if (!job.getOrganization().getId().equals(organization.getId())) {
            return ResponseObject.message("this job not belong to this organization");
        }
